...
#endif
	m_bBitmapStretch = TRUE;
	m_bMouse_Event = FALSE;
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	m_ptCurrentPos.SetPoint(point.x, point.y);
} // End of OnMouseMove
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	int nXTextPos, nYTextpos;
	nXTextPos = nYTextpos = 0;
	// Checkbox?
	if (m_bIsCheckBox)
	{
	...
	nXTextPos = itemRect.left;
	nYTextpos = itemRect.top;
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	else
	{
		if (m_bBitmapStretch)
		{
			...
			::StretchBlt(pDC->m_hDC, 0, 0, rect.Width(), rect.Height(), hdcMem, 0, 0, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, SRCCOPY);

			::SetStretchBltMode(pDC->m_hDC, HALFTONE);
			::StretchBlt(pDC->m_hDC, 0, 0, rect.Width(), rect.Height(), hdcBmpMem, 0, 0, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, SRCCOPY);
		}
		else
		{
...
void CButtonST::OnMouseHover(UINT nFlags, CPoint point)
{
	m_bMouse_Event = TRUE;

	NMHDR nmhdr;
	nmhdr.code     = WM_USER + 5;
	nmhdr.idFrom   = GetDlgCtrlID();
	nmhdr.hwndFrom = GetSafeHwnd();
