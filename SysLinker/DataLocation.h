...
// CDataLocation 명령 대상입니다.

class CDataLocation : public CObject
{
public:
	CDataLocation(short sLcType , CString strKey);
	virtual ~CDataLocation();
	CString GetLocationString();

	BOOL IsSame(CDataLocation * pSrc);
	BOOL CopyData(CDataLocation * pSrc);
	...
	SIMPLE_FUNC_IMPL(CString, MapKey, m_strKey);
