							  ...
							  XLISTCTRLDATA *pXLCD)
{
	...
	if (pXLCD[nSubItem].nProgressPercent > 0)
	{
		...
		//pDC->FillSolidRect(RightRect, m_crHighLight);
						  ...
						  XLISTCTRLDATA *pXLCD)
{
	...
	if (!str.IsEmpty())
	{
		...
		if (pXLCD && (pXLCD[nSubItem].bBold
			|| pXLCD[nSubItem].nFontSize > 0
			|| pXLCD[nSubItem].strFontName != _T("")
			))
		{
			...
			if (font)
			{
				LOGFONT lf;
				font->GetLogFont(&lf);
				if (pXLCD[nSubItem].nFontSize > 0)
					lf.lfHeight = pXLCD[nSubItem].nFontSize;
				if (pXLCD[nSubItem].bBold)
					lf.lfWeight = FW_BOLD;
				if (pXLCD[nSubItem].strFontName != _T(""))
					_tcscpy_s(lf.lfFaceName, 32, pXLCD[nSubItem].strFontName);
	...
	else
	{
		...
		else
		{
			if (pXLCD[nSubItem].nCheckedState >= 0)
			{
				...
				XNOTIFY_CHECK xnData;
				xnData.dispinfo.hdr.hwndFrom = m_hWnd;
				xnData.dispinfo.hdr.idFrom = GetDlgCtrlID();
				xnData.dispinfo.hdr.code = LVN_XL_CHECK;

				xnData.dispinfo.item.mask = LVIF_TEXT;
				xnData.dispinfo.item.iItem = nItem;
				xnData.dispinfo.item.iSubItem = nSubItem;
				xnData.nCheckState = nChecked;
				xnData.dwItemData = GetItemData(nItem);;
				lresult = GetParent()->SendMessage(WM_NOTIFY, GetParent()->GetDlgCtrlID(), (LPARAM)&xnData);
				lresult = GetOwner()->SendMessage(WM_NOTIFY, GetParent()->GetDlgCtrlID(), (LPARAM)&xnData);
...
///////////////////////////////////////////////////////////////////////////////
// InsertItem
int CXListCtrl::InsertColumn(_In_ int nCol, _In_z_ LPCTSTR lpszColumnHeading
	, _In_ int nFormat, _In_ int nWidth, _In_ int nSubItem)
{
	int nIdx = -1;
	nIdx = CListCtrl::InsertColumn(nCol,lpszColumnHeading,nFormat,nWidth,nSubItem);
	SetSortType(nCol, SRT_STRING);
	return nIdx;
}
...
int CXListCtrl::InsertItem(const LVITEM* pItem)
{
	...
	for(i =0 ; i < nColCnt ; i ++)
	{
		pXLCD[i].crText       = m_crWindowText;
		pXLCD[i].crBackground = m_crWindow;
		pXLCD[i].nImage       = -1;
		pXLCD[i].nFontSize = m_nWindowFontSize;
		pXLCD[i].strFontName = m_strWindowFont;
	}
	...
	pXLCD[0].nFontSize = m_nWindowFontSize;
	pXLCD[0].strFontName = m_strWindowFont;
						   ...
						   COLORREF crBackground)
{
	...
	XLISTCTRLDATA *pXLCD = new XLISTCTRLDATA [nColCnt];
	ASSERT(pXLCD);
	if (!pXLCD)
		return -1;
...
///////////////////////////////////////////////////////////////////////////////
// SetItemText
BOOL CXListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText , BOOL bReDraw )
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
					...
					COLORREF crText, COLORREF crBackground)
{
	...
	if (pXLCD)
	{
		...
		pXLCD[nSubItem].nFontSize = m_nWindowFontSize;
		pXLCD[nSubItem].strFontName = m_strWindowFont;
	}
...
BOOL CXListCtrl::DeleteAllItems()
{
	...
	n = m_arrDirection.GetSize();
	for (i = 0; i < n; i++)
	{
		m_arrDirection.SetAt(i, -1);
	}
...
///////////////////////////////////////////////////////////////////////////////
// SetEdit
BOOL CXListCtrl::SetEdit(int nItem, int nSubItem , bool bEdit , bool bNumberic)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	pXLCD[nSubItem].bEdit = bEdit;
	pXLCD[nSubItem].bNumberic = bNumberic;
...
///////////////////////////////////////////////////////////////////////////////
// SetCheckbox
BOOL CXListCtrl::SetCheckbox(int nItem, int nSubItem, int nCheckedState, BOOL bReDraw )
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
...
BOOL CXListCtrl::OnColumnClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	nTempState = (nCheckedState == 1) ? 2 : 1;;
	// 0 = no checkbox
	if (nCheckedState != XHEADERCTRL_NO_IMAGE)
	{
		...
		xnData.dispinfo.hdr.hwndFrom = m_hWnd;
		xnData.dispinfo.hdr.idFrom = GetDlgCtrlID();
		xnData.dispinfo.hdr.code = LVN_XL_HEAD_CHECK;

		xnData.dispinfo.item.mask = LVIF_TEXT;
		xnData.dispinfo.item.iItem = nItem;
		xnData.dispinfo.item.iSubItem = nSubItem;
		xnData.nCheckState = nTempState-1;
		xnData.dwItemData = 0;
		lresult = GetParent()->SendMessage(WM_NOTIFY, GetParent()->GetDlgCtrlID(), (LPARAM)&xnData);
	...
	{
		// Sortting
		int nAsc = m_arrDirection.GetAt(nSubItem);
		if (nAsc <= 0)
			nAsc = 1;
		else
			nAsc = 0;
		...
		if (m_pCombo)
		{
			...
			if (i != LB_ERR)
			{
					...
					(m_nComboSubItem >= 0 && m_nComboSubItem < GetColumns()))
				{
					...
					XNOTIFY_COMBO_SELECT xnData;
					//LV_DISPINFO dispinfo;
					xnData.dispinfo.hdr.hwndFrom = m_hWnd;
					xnData.dispinfo.hdr.idFrom = GetDlgCtrlID();
					xnData.dispinfo.hdr.code = LVN_XL_COMBO_SELECT;

					xnData.dispinfo.item.mask = LVIF_TEXT;
					xnData.dispinfo.item.iItem = m_nComboItem;
					xnData.dispinfo.item.iSubItem = m_nComboSubItem;
					//xnData.dispinfo.item.pszText = plvItem->pszText;
					//xnData.dispinfo.item.cchTextMax = strlen(plvItem->pszText);
					xnData.nIdxComboItem = i;
					GetParent()->SendMessage( WM_NOTIFY, GetParent()->GetDlgCtrlID(), (LPARAM)&xnData );
				}
			}
...
void CXListCtrl::DrawEdit(int nItem, int nSubItem)
{
	...
	m_pEdit = new CXEdit(this, m_strInitialString ,pXLCD[nSubItem].bNumberic);
...
LRESULT CXListCtrl::OnXEditKillFocus(WPARAM, LPARAM)
{
	...
	XNOTIFY_EDIT_END xnData;
	xnData.dispinfo.hdr.hwndFrom = m_hWnd;
	xnData.dispinfo.hdr.idFrom = GetDlgCtrlID();
	xnData.dispinfo.hdr.code = LVN_XL_EDIT_END;

	xnData.dispinfo.item.mask = LVIF_TEXT;
	xnData.dispinfo.item.iItem = m_nEditItem;
	xnData.dispinfo.item.iSubItem = m_nEditSubItem;
	xnData.nIdxEditItem = m_nEditItem;
	GetParent()->SendMessage( WM_NOTIFY, GetParent()->GetDlgCtrlID(), (LPARAM)&xnData );
...
void CXListCtrl::Sort(int nSubItem, BOOL bAsc)
{
	int i = 0 , nSortType = SRT_STRING; 
	XLISTCTRLDATA *pXLCD = nullptr;
	...
	SetRedraw(FALSE);
	m_bSorting = TRUE;
	if (m_arrSortType.GetSize() <= nSubItem)
		nSortType = SRT_STRING;
	else
		nSortType = m_arrSortType.GetAt(nSubItem);
	for (i = 0; i < nCnt; i++)
	{
		pXLCD = (XLISTCTRLDATA *)CListCtrl::GetItemData(i);
		ASSERT(pXLCD);
		if (pXLCD == nullptr)
			continue;
		pXLCD->nSortType = nSortType;
		pXLCD->nSortDirection = bAsc;
		pXLCD->strText = GetItemText(i, nSubItem);
	}
	CListCtrl::SortItems(&CompareFunc, bAsc);
	m_arrDirection.SetAt(nSubItem, bAsc);
	m_bSorting = FALSE;
	SetRedraw(TRUE);
}
...
int CALLBACK CXListCtrl::CompareFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	switch (pXLCD1->nSortType)
	{
	case  SRT_INT:
		nRet = (_wtoi(pXLCD1->strText.GetBuffer()) - _wtoi(pXLCD2->strText.GetBuffer()))*sOrder;
		return nRet;
	case  SRT_FLOAT:
		nRet = (_wtof(pXLCD1->strText.GetBuffer()) < _wtof(pXLCD2->strText.GetBuffer()) ? -1 : 1)*sOrder;
		return nRet;
//	case  SRT_DATETIME:
// 		if (dt1.ParseDateTime(pXLCD1->strText) && dt2.ParseDateTime(pXLCD2->strText))
// 			return (dt1 < dt2 ? -1 : 1)*sOrder;
// 		else
// 			return 0;
	default:
		return pXLCD1->strText.CompareNoCase(pXLCD2->strText)*sOrder;
	}
}


void CXListCtrl::OnLvnGetdispinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 		return;
// 
// 	nSize = pXLCD[pItem->iSubItem].strText.GetLength() + 2;
// 	if (pItem->mask & LVIF_TEXT)
// 	{
// 		nSize = str.GetLength();
// 		if (nSize > 0)
// 			wcscpy_s(pItem->pszText, nSize + 2, pXLCD[pItem->iSubItem].strText);
// 	}
