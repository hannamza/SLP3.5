...
void CXHeaderCtrl::DrawCtrl(CDC* pDC)
{
	...
	{
		pDC->FillSolidRect(&rectClip, m_crHeader);
		pDC->SetBkColor(m_crHeader);
	}
...
//  			pDC->FrameRect(&chkboxrect, &brush);
			...
			COLORREF rgbBackground = pDC->SetBkColor(RGB(255, 0, 0));
			pDC->FillSolidRect(&chkboxrect, RGB(255, 0, 0));
			// draw border
			...
			)
		{
			...
			if (font)
			{
				...
				if (m_bBold)
					lf.lfWeight = FW_BOLD;
				if (m_strFontName != _T(""))
					_tcscpy_s(lf.lfFaceName, 32, m_strFontName);
...
///////////////////////////////////////////////////////////////////////////////
// OnInsertItem
LRESULT CXHeaderCtrl::OnInsertItem(WPARAM wp, LPARAM lp)
{
	int nIdx = (int)lp;
	return Default();
}
...
CString CXHeaderCtrl::GetItemText(int nSubItem)
{
	HDITEM itemData;
	GetItem(nSubItem, &itemData);
